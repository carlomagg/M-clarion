            ...
            <div ref={menuRef} className='relative'>
                <button onClick={() => setIsExpanded(!isExpanded)} type="button" className='border-[.5px] border-[#CCC] rounded-full pr-1 flex gap-1'>
                    <div className='border-2 border-text-pink rounded-full flex items-center justify-center'>
                        <span className='inline-grid place-items-center w-8 h-8 p-[2px] border-2 border-white rounded-full bg-[#FF69B4] text-white font-semibold text-sm capitalize'>{initials.toUpperCase()}</span>
                    </div>
                    <img src={chevronDownIcon} alt="chevron icon" className='self-center' />
                </button>
                {isExpanded && (
                    <div className="absolute top-full right-0 mt-1 bg-[#F5F5F5] rounded-2xl z-50 shadow-lg w-[280px] overflow-hidden border border-black">
                        {/* User Info Section */}
