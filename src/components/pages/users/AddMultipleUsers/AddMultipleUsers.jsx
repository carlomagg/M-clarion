...
function AddMultipleUsers() {
    ...
    const { mutate, isPending } = useAddUsers({
        ...
        onError: (error) => {
            ...
            dispatchMessage('failed', error.response?.data?.message || 'Failed to add users');
        }
    });

    useEffect(() => {
        if (isPending) {
            dispatchMessage('processing', 'Adding users');
        }
    }, [isPending]);
        ...
        <div className='p-10 pt-4 max-w-7xl flex flex-col gap-6'>
            ...
            <PageHeader>
                <div className='flex gap-3 items-center'>
                    <h2 className='text-[12px] font-medium'>Upload CSV File</h2>
                </div>
            </PageHeader>
