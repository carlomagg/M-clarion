...
export function CustomCard({title, data}) {
    return (
        <Card title={title} filterableByPeriod={false} classes="px-6 pt-6 pb-4 flex-1 max-w-[250px] items-start">
            <div className="flex gap-3">
                <span className="font-bold">{data.value}</span>
                <Chip color={data.statusColor} text={data.comment} />
            </div>
        </Card>
    );
}
