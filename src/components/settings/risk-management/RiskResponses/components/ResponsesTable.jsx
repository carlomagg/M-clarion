...
import { useDeleteRiskResponse } from "../../../../../queries/risks/risk-responses";
import { useQueryClient } from "@tanstack/react-query";
import useDispatchMessage from "../../../../../hooks/useDispatchMessage";
import useConfirmedAction from "../../../../../hooks/useConfirmedAction";

export default function RiskResponsesTable({items}) {
    const [showModal, setShowModal] = useState(false);
    const {confirmAction, confirmationDialog} = useConfirmedAction();

    // mutations
    const {isPending, mutate: deleteResponse} = useDeleteRiskResponse({onSuccess, onError});

    const queryClient = useQueryClient();
    const dispatchMessage = useDispatchMessage();
    useEffect(() => {
        let text = 'Deleting Risk Response';
    ...
    function createRecordOptions(record) {
        const options = [
            ...
            {text: 'Delete', type: 'action', action: () => confirmAction(() => deleteResponse({id: record.id}))},
            ...
            <div className='w-full'>
                <div className=' rounded-lg text-[#3B3B3B] text-sm'>
                    <header className='px-4 border-b border-b-[#B7B7B7] flex gap-4'>
                        <span className='py-4 flex-[1_0]'>Response</span>
