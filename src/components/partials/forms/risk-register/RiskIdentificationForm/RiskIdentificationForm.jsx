...
function RiskIdentificationForm({mode, toggleAIAssistance, setRiskName, setRiskCategory, setRiskClass}) {
    ...
    useEffect(() => {
        let text = isAddingRisk ? 'Adding Risk' : (isUpdatingRisk ? 'Updating Risk' : ((isSavingNewToDraft || isSavingExistingToDraft) && 'Saving To Draft'));
    ...
    const riskCategories = riskCategoriesQuery.data.map(cat => ({id: cat.id, text: cat.category}));
