...
function LicenseManagement() {
    ...
    const handleUserSelection = async (userId, isSelected) => {
        if (isSelected) {
            ...
            // Update permissionsToRemove to remove permissions only relevant to this user
            // This is more complex and requires checking other selected users
            const remainingUsers = selectedUsers.filter(id => id !== userId);
            
            if (remainingUsers.length === 0) {
                // If no users left, clear permissions to remove
                setPermissionsToRemove([]);
            } else {
                // Otherwise, keep only permissions that remain relevant
                setPermissionsToRemove(prev => 
                    prev.filter(permId => 
                        remainingUsers.some(uId => 
                            userPermissions[uId]?.includes(permId)
                        )
                    )
                );
            }
    ...
    useEffect(() => {
        if (selectedUsers.length > 0) {
            ...
            if (allUsersLoaded) {
                ...
                // Update permissionsToRemove with all permissions from all selected users
                const allSelectedPermissions = selectedUsers.reduce((permissions, userId) => {
                    return [...permissions, ...(userPermissions[userId] || [])];
                }, []);
                
                // Remove duplicates
                const uniquePermissions = [...new Set(allSelectedPermissions)];
                
                console.log('Setting permissionsToRemove:', uniquePermissions);
                setPermissionsToRemove(uniquePermissions);
            }
        }
    ...
    return (
        <div className="p-6">
            ...
            {!showRemovePermissions ? (
                <>
                    {/* License Section - Now full width */}
                                ...
                                isLoadingQueue ? (
                                    ...
                                    filteredQueueUsers.map((user) => (
                                        <div
                                            ...
                                            onClick={() => {
                                                // Add the user to selected users directly when clicking the row
                ...
                <>
                    ...
                    <div className="mb-6">
                        <h2 className="text-xl font-semibold mb-4 text-gray-800">Select Users to Remove Permissions From</h2>
                                ...
                                (() => {
                                    ...
                                    if (anyUserHasPermissions) {
                                        return permissions.map((section, sectionIndex) => {
                                            // Get all permissions from this section
                                            const allSectionPermissions = section.permissionGroups?.flatMap(group => 
                                                group.permissions || []
                                            ) || [];
                                            
                                            // Filter to only show permissions assigned to at least one selected user
                                            const assignedSectionPermissions = allSectionPermissions.filter(permission => {
                                                const permissionId = parseInt(permission.permission_id, 10);
