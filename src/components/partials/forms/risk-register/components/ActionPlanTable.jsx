                    ...
                    <ul className='flex flex-col'>
                        {
                            plans.map((plan, i) => {
                                return (
                                    <li key={i}>
                                        <ActionPlanRow plan={plan} serialNo={i+1} options={createRecordOptions(i)} onPlanChange={onPlanChange} editable={editable} users={users} registerStatuses={registerStatuses} aiSuggestionData={aiSuggestionData} />
                                    </li>
                                );
                            })
                        }
...
function ActionPlanRow({plan, serialNo, onPlanChange, options, editable, users, registerStatuses}) {
    return (
        <div className="relative">
            <div className='px-4 flex gap-4 items-center'>
                ...
                <span className='py-4 flex-[3_0]'>
                    {
                        editable ?
                        <input type="text" name='action' value={plan.action} onChange={(e) => onPlanChange(e, true, i)} placeholder='Enter action' className='p-2 outline-none rounded-lg border border-border-gray w-full' /> :
                ...
                <span className='py-4 flex-[2_0]'>
                    {
                        editable ?
                        <NameDropdown names={users} selected={plan.assigned_to.id} onChange={(e) => onPlanChange(e, true, i)} /> :
                ...
                <span className='py-4 flex-[1_0]'>
                    {
                        editable ?
                        <input type="date" name='due_date' value={plan.due_date} onChange={(e) => onPlanChange(e, true, i)} className='p-2 outline-none rounded-lg border border-border-gray w-full' /> :
                ...
                <span className='py-4 flex-[1_0]'>
                    {
                        editable ?
                        <StatusDropdown noLabel={true} statuses={registerStatuses} selected={plan.status_id} onChange={(e) => onPlanChange(e, true, i)} /> :
